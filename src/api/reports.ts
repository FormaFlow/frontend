...
export default {
  ...
  grouped(params: GroupedParams) {
    return client.post<{ groups: { category: string, value: number }[] }>('/reports/grouped', params)
  },
  ...
  getWeeklySummary(formId: string) {
    return client.get<any>(`/reports/weekly-summary?form_id=${formId}`)
  },

  getMonthlySummary(formId: string, month?: string) {
    const query = month ? `&month=${month}` : ''
    return client.get<any>(`/reports/monthly-summary?form_id=${formId}${query}`)
  },

  getPredefinedReport(type: 'budget' | 'medicine' | 'weight', params: any = {}) {
    const query = new URLSearchParams(params).toString()
    return client.get<any>(`/reports/predefined/${type}?${query}`)
  },
